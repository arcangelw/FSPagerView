...
open class FSPagerView: UIView, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    open var scrollDirection: FSPagerView.ScrollDirection = .horizontal {
        didSet {
            collectionViewLayout.invalidateAndReprepare()
        }
    }
    ...
    open var itemSize: CGSize = automaticSize {
        didSet {
            itemSizeMode = .size(itemSize)
            collectionViewLayout.invalidateAndReprepare()
        }
    }

    /// The item size of the pager view. When the value of this property is FSPagerView.automaticAspectRatio, the items fill the entire visible area of the pager view. Default is FSPagerView.automaticAspectRatio.
    @IBInspectable
    open var aspectRatio: CGFloat = automaticAspectRatio {
        didSet {
            itemSizeMode = .aspectRatio(aspectRatio)
            collectionViewLayout.invalidateAndReprepare()
        }
    }

    /// The item size of the pager view. When the value of this property is FSPagerView.automaticSize, the items fill the entire visible area of the pager view. Default is FSPagerView.automaticSize.
    open var itemSizeMode: ItemSizeMode = .size(automaticSize) {
        didSet {
            collectionViewLayout.invalidateAndReprepare()
        }
    }
    ...
    @objc public var pagerCollectionView: UICollectionView {
    ...
    fileprivate var registeredCellIdentifiers: Set<String> = .init()
    ...
    ///   - cellType: cellType
    /// - Returns: A valid UICollectionViewCell object.
    public final func dequeueReusableCell<T: UICollectionViewCell>(at index: Int, cellType: T.Type = T.self) -> T {
        ...
        let indexPath = IndexPath(item: index, section: dequeingSection)
        let bareCell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath)
    ...
    open func scrollToItem(at index: Int, animated: Bool) -> Bool {
        guard index < numberOfItems else { return false }
...
public extension FSPagerView {
    ...
    /// Requests that FSPagerView use the default value for a given size.
    static let automaticAspectRatio: CGFloat = 0

    enum ItemSizeMode {
        case size(_ size: CGSize)
        case aspectRatio(_ aspectRatio: CGFloat)
        case insets(_ insets: UIEdgeInsets)
